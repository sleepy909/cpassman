...
function IdentificationDesDroits($groupes_visibles_user,$groupes_interdits_user,$is_admin,$id_fonctions,$refresh){    
    ...
    if ( $is_admin == 1 ){
        ...
        $res = mysql_query("SELECT id FROM nested_tree");
        ...
        $groupes_visibles = array();
        $groupes_interdits = explode(';',$groupes_interdits_user);
        $_SESSION['is_admin'] = $is_admin;
        ...
        foreach($fonctions_associees as $fonc_id){
            if ( !empty($fonc_id) ){
                $res = mysql_query("SELECT groupes_visibles,groupes_interdits FROM functions WHERE id=".$fonc_id);
