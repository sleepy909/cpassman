...
{    
    case "new_item":
        ...
        $res = mysql_query("SELECT COUNT(*) FROM items WHERE label = '".mysql_real_escape_string(stripslashes(($_POST['label'])))."' AND inactif=0");
        $data = mysql_fetch_row($res);
        if ( $data[0] != 0 ) 
            echo 'alert(\''.$txt['error_item_exists'].'\');';
        else {
            //ajouter élément
            $sql = "INSERT INTO items VALUES (NULL,'".mysql_real_escape_string(stripslashes($_POST['label']))."','".addslashes($_POST['desc'])."','".encrypt($_POST['pw'])."','".mysql_real_escape_string(stripslashes(($_POST['url'])))."','".$_POST['categorie']."','".$_POST['personnel']."','".mysql_real_escape_string(stripslashes(($_POST['login'])))."','0','".$_POST['restricted_to']."')";
            $res = mysql_query($sql) or die('Erreur SQL !<br>'.$sql.'<br>'.mysql_error());
            $new_id=mysql_insert_id();
            //log
            mysql_query("INSERT INTO log_items VALUES ('".$new_id."','".mktime(date('H'),date('i'),date('s'),date('m'),date('d'),date('y'))."','".$_SESSION['id']."','Création','')");
            //annoncer par email
            if ( $_POST['annonce'] == 1 ){
    ...
    case "update_item":
        ...
        //MAJ l'item 
        $sql = "UPDATE items SET label = '".mysql_real_escape_string(stripslashes(($_POST['label'])))."', description = '".addslashes($_POST['description'])."', pw = '".encrypt($pw_recu)."', login = '".mysql_real_escape_string(stripslashes(($_POST['login'])))."', url = '".mysql_real_escape_string(stripslashes(($_POST['url'])))."', id_tree = '".mysql_real_escape_string($_POST['categorie'])."', perso = '".$_POST['perso']."', restricted_to = '".$_POST['restricted_to']."' WHERE id='".$_POST['id']."'";
        mysql_query($sql) or die($sql.'  =>  '.mysql_error());
        
        //identifier les différences
        if ( $data['label'] != $_POST['label'] ) mysql_query("INSERT INTO log_items VALUES ('".$_POST['id']."','".mktime(date('H'),date('i'),date('s'),date('m'),date('d'),date('y'))."','".$_SESSION['id']."','Modification','Label : ".$data['label']." => ".mysql_real_escape_string(stripslashes(($_POST['label'])))."')");
        if ( $data['login'] != $_POST['login'] ) mysql_query("INSERT INTO log_items VALUES ('".$_POST['id']."','".mktime(date('H'),date('i'),date('s'),date('m'),date('d'),date('y'))."','".$_SESSION['id']."','Modification','login : ".$data['login']." => ".mysql_real_escape_string(stripslashes(($_POST['login'])))."')");
        if ( $data['url'] != $_POST['url'] ) mysql_query("INSERT INTO log_items VALUES ('".$_POST['id']."','".mktime(date('H'),date('i'),date('s'),date('m'),date('d'),date('y'))."','".$_SESSION['id']."','Modification','Url : ".$data['url']." => ".mysql_real_escape_string(stripslashes($_POST['url']))."')");
        if ( $data['description'] != $_POST['description'] ) mysql_query("INSERT INTO log_items VALUES ('".$_POST['id']."','".mktime(date('H'),date('i'),date('s'),date('m'),date('d'),date('y'))."','".$_SESSION['id']."','Modification','Description.')");
        if ( $data['perso'] != $_POST['perso'] ) mysql_query("INSERT INTO log_items VALUES ('".$_POST['id']."','".mktime(date('H'),date('i'),date('s'),date('m'),date('d'),date('y'))."','".$_SESSION['id']."','Modification','Personnel : ".$data['perso']." => ".$_POST['perso']."')");
        if ( $data['id_tree'] != mysql_real_escape_string($_POST['categorie']) ) mysql_query("INSERT INTO log_items VALUES ('".$_POST['id']."','".mktime(date('H'),date('i'),date('s'),date('m'),date('d'),date('y'))."','".$_SESSION['id']."','Modification','Catégorie : ".$data['id_tree']." => ".mysql_real_escape_string($_POST['categorie'])."')");
        if ( $data['pw'] != encrypt($pw_recu) ) mysql_query("INSERT INTO log_items VALUES ('".$_POST['id']."','".mktime(date('H'),date('i'),date('s'),date('m'),date('d'),date('y'))."','".$_SESSION['id']."','Modification','Mot de passe changé')");
        
        //recharger les nouvelles données
            $sql = "SELECT * FROM items i, log_items l WHERE i.id=".$_POST['id']." AND l.id_item = i.id AND l.action = 'Création'";
            $res = mysql_query($sql) or die('Erreur SQL !<br>'.$sql.'<br>'.mysql_error());
            $data_item = mysql_fetch_array($res);
            $historique = "";
            $res1 = mysql_query("SELECT * FROM log_items WHERE id_item=".$_POST['id']);
            while ( $data1 = mysql_fetch_array($res1) ){
                $res2 = mysql_query("SELECT login FROM users WHERE id=".$data1['id_user']);
            ...
            foreach($liste as $elem){
                if ( !empty($elem) ){
                    $data2 = mysql_fetch_row(mysql_query("SELECT login FROM users WHERE id=".$elem));
    ...
    case "show_details_item":
        ...
        if ( in_array($_SESSION['id'],$restricted_to) ) $restriction_active = false;
        ...
        if ( ( in_array($access[0],$_SESSION['groupes_visibles']) OR $_SESSION['is_admin'] == 1 ) 
            AND  ( $data_item['perso']==0 OR ($data_item['perso']==1 AND $data_item['id_user'] == $_SESSION['id'] ) )  
            ...
            //echo 'document.getElementById(\'item_menu_notactivated\').style.display = "none";';
            //echo 'document.getElementById(\'item_menu\').style.display = "inline";';
            ...
            if ($data_item['id_user'] == $_SESSION['id'] || $_SESSION['is_admin'] == 1 )
                echo '$(\'#contextMenuContent\').enableContextMenuItems(\'#del_item\');';//echo 'document.getElementById(\'icon_del_mdp\').style.display = "";';
            else
                echo '$("#contextMenuContent").disableContextMenuItems("#del_item");';//echo 'document.getElementById(\'icon_del_mdp\').style.display = "none";';
        }else{
    ...
    case "del_item":
        $sql = "UPDATE items SET inactif = '1' WHERE id = ".$_POST['id'];
        mysql_query($sql) or die($sql.'  =>  '.mysql_error());
        //log
        mysql_query("INSERT INTO log_items VALUES ('".$_POST['id']."','".mktime(date('H'),date('i'),date('s'),date('m'),date('d'),date('y'))."','".$_SESSION['id']."','Suppression','')");
        //recharger
        echo 'window.location.href = "index.php?page=items&group='.$_POST['groupe'].'";';
    break;
    
    case "new_rep":        
        $res = mysql_query("SELECT COUNT(*) FROM nested_tree WHERE title = '".mysql_real_escape_string(stripslashes(($_POST['title'])))."'");
        $data = mysql_fetch_row($res);
        if ( $data[0] != 0 ) 
            echo 'alert(\'Ce groupe existe déjà !\');';
        else {
            $sql = "INSERT INTO nested_tree VALUES (NULL,'".$_POST['groupe']."','".mysql_real_escape_string(stripslashes(($_POST['title'])))."','','','','0','0')";
            mysql_query($sql) or die('Erreur SQL !<br>'.$sql.'<br>'.mysql_error());
            $new_id=mysql_insert_id();
            
            //ajouter complexité
            mysql_query("INSERT INTO misc VALUES ('complex','".$new_id."','".$_POST['complexite']."')");
    ...
    case "update_rep":    
        $sql = "UPDATE nested_tree SET title = '".mysql_real_escape_string(stripslashes(($_POST['title'])))."' WHERE id='".$_POST['groupe']."'";
        mysql_query($sql) or die($sql.'  =>  '.mysql_error());
        
        //editer complexité
        mysql_query("UPDATE misc SET valeur = '".$_POST['complexite']."' WHERE intitule = '".$_POST['groupe']."' AND type = 'complex'");
    ...
    case "delete_rep":
        ...
        if ( count($tmp) == 1 ){            
            //supprimer le groupe
            mysql_query("DELETE FROM nested_tree WHERE id='".$tmp[0]."'");
            
            //supprimer les catégories associées
           $res = mysql_query("SELECT id FROM nested_tree WHERE parent_id=".$tmp[0]);
           while ( $data = mysql_fetch_row($res) ){
                mysql_query("DELETE FROM nested_tree WHERE id='".$data[0]."'");
                //suprimer toutes les entrées associées
                $res = mysql_query("SELECT id FROM items WHERE id_tree='".$tmp[0].";".$data[0]."'");
                while ( $data = mysql_fetch_row($res) ){
                    $sql = "UPDATE items SET inactif = '1' WHERE id = ".$data[0];
                    mysql_query($sql) or die($sql.'  =>  '.mysql_error());
                    //log
                    mysql_query("INSERT INTO log_items VALUES ('".$data[0]."','".mktime(date('H'),date('i'),date('s'),date('m'),date('d'),date('y'))."','".$_SESSION['id']."','Suppression','')");
                }
            } 
        }else{
            mysql_query("DELETE FROM categories WHERE id='".$tmp[1]."'");
            //suprimer toutes les entrées associées
            $res = mysql_query("SELECT id FROM items WHERE id_categorie='".$tmp[0].";".$tmp[1]."'");
            while ( $data = mysql_fetch_row($res) ){
                $sql = "UPDATE items SET inactif = '1' WHERE id = ".$data[0];
                    mysql_query($sql) or die($sql.'  =>  '.mysql_error());
                    //log
    ...
    case 'save_position':
        require_once ("NestedTree.class.php");
        mysql_query("UPDATE nested_tree SET parent_id = '".$_POST['destination']."' WHERE id='".$_POST['source']."'");
       // echo "UPDATE nested_tree SET parent_id = '".$_POST['destination']."' WHERE id='".$_POST['source']."'";
        $tree = new NestedTree('nested_tree', 'id', 'parent_id', 'title');
        $tree->rebuild();
    break;
    ...
    case 'lister_items_groupe':
        ...
        $res1 = mysql_query("SELECT * FROM items WHERE inactif = 0 AND id_tree=".$_POST['id']." ORDER BY label");
        while ( $data1 = mysql_fetch_array($res1) ){
    ...
    case "recup_complex":
        $res = mysql_query("SELECT valeur FROM misc WHERE type='complex' AND intitule = '".$_POST['groupe']."'");
        $data = mysql_fetch_row($res);
        ...
        $visibilite = "";
        $res = mysql_query("SELECT valeur FROM misc WHERE type='visibilite' AND intitule = '".$_POST['groupe']."'");
        $data = mysql_fetch_row($res);
        $tab = explode(';',$data[0]);
        foreach($tab as $elem){
            //rechercher l'itnitulé du groupe
            $data = mysql_fetch_row(mysql_query("SELECT title FROM functions WHERE id = '".$elem."'"));
            if ( !empty($data[0]) ){
...
function RecupDroitCreationSansComplexite($groupe){
    ...
    $data = mysql_fetch_row(mysql_query("SELECT bloquer_creation,bloquer_modification FROM nested_tree WHERE id = '".$groupe."'"));
    ...
    foreach($tst as $t){
        ...
        if ( in_array($t->id,$_SESSION['groupes_visibles']) ){
            //récupérer les ITEMS liés à ce groupe
                $items = array();
                $text_items = "";
                $res = mysql_query("SELECT * FROM items where inactif=0 AND id_tree = ".$t->id);
