...
$liste_fonctions = "";
$res = mysql_query("SELECT * FROM functions ORDER BY title ASC");
...
echo '
    <form name="form_groupes" method="post" action="">
        ...
        <table cellspacing="0" style="margin-top:10px;">
            ...
            foreach($tst as $t){
                //récup $t->parent_id
                $res = mysql_query("SELECT title FROM nested_tree WHERE id = ".$t->parent_id);
                $data = mysql_fetch_row($res);
                if ( $t->nlevel == 1 ) $data[0] = "Racine";
                
                //récup les droits associés à ce groupe
                $tab_droits=array();
                $res1 = mysql_query("SELECT fonction_id  FROM rights WHERE authorized=1 AND tree_id = ".$t->id);
                ...
                $res = mysql_query("SELECT valeur FROM misc WHERE type='complex' AND intitule = ".$t->id);
                ...
                echo '<tr class="ligne'.($x%2).'">
                        ...
                        $data3 = mysql_fetch_row(mysql_query("SELECT bloquer_creation,bloquer_modification FROM nested_tree WHERE id = ".$t->id));
                        echo '
                        <td align="center">
