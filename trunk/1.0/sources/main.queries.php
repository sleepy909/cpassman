...
switch($_POST['type'])
{
    case "change_pw":
        ...
        if ( in_array(md5($_POST['new_pw']),$tmp) ){
            ...
            $sql="UPDATE users SET pw = '".md5($_POST['new_pw'])."', last_pw_change = '".mktime(0,0,0,date('m'),date('d'),date('y'))."', last_pw = '".implode(';',$tmp)."' WHERE id = ".$_SESSION['id'];
    ...
    case "identify_user":
        ...
        if ( md5($_POST['pw']) == $data['pw'] ) {
            ...
            //Save account in SESSION
                ...
                $_SESSION['groupes_visibles'] = $data['groupes_visibles'];
                $_SESSION['groupes_interdits'] = $data['groupes_interdits'];
                $_SESSION['fonction_id'] = $data['fonction_id'];

            // Update table
            $sql = "UPDATE users SET key_tempo='".$_SESSION['cle_session']."', last_connexion='".mktime(date("h"),date("i"),date("s"),date("m"),date("d"),date("Y"))."' WHERE id=".$data['id'];
