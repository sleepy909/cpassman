...
if ( (isset($_POST['menu_action']) && $_POST['menu_action'] == "deconnexion") || (isset($_GET['session']) && $_GET['session'] == "expiree") ){
    ...
    $sql = "UPDATE users SET key_tempo='' WHERE id=".$_SESSION['id'];
...
if ( !empty($_SESSION['fin_session']) ) {
    $res = mysql_query("SELECT key_tempo FROM users WHERE id=".$_SESSION['id']);
    $data_session = mysql_fetch_row($res);
}else $data_session[0] = "";

if ( isset($_SESSION['id']) && ( empty($_SESSION['fin_session']) || $_SESSION['fin_session'] < time() || empty($_SESSION['cle_session']) || $_SESSION['cle_session'] != $data_session[0] ) ){
    // Update table by deleting ID
    $sql = "UPDATE users SET key_tempo='' WHERE id=".$_SESSION['id'];
...
<html xmlns="http://www.w3.org/1999/xhtml" xml:lang="en" lang="en">
    ...
    if ( isset($_SESSION['autoriser']) && $_SESSION['autoriser'] == true ){
        ...
        <form method="post" name="main_form" action="">
            ...
            <input type="hidden" name="form_user_id" id="form_user_id" value="'.$_SESSION['id'].'" />
