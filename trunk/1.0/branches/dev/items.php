...
$tmp = "";
$res_utilistateurs = mysql_query("SELECT id,login,email FROM users ORDER BY login ASC");
...
<div id="div_items">';
    ...
    <div id="sidebar" class="sidebar" style="border-right:1px solid #FF8000;overflow-y:auto;">';
        ...
        foreach($tst as $t){
            ...
            $res = mysql_query("SELECT COUNT(*) FROM items WHERE inactif=0 AND id_tree = ".$t->id);
            ...
            if ( $cpt_total == 0 ) {
                 ...
                 <li id="li_'.$t->id.'"><span class="folder">&nbsp;</span>
                    ...
                    '<a class="folder_item" onclick="ListerItems(\''.$t->id.'\');">'.$t->title.'</a> ('.$nb_items.')' : 
                    '<span class="folder_item">'.$t->title.'</span>',
                    '</div>';
